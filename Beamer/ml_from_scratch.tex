...
Before we really dive in, I want to tell you a bit about my talk today, and to do so, we're going to start with two things that help tremendously in successfully applying machine learning... (1) Understanding the math at play: a great way to build that intuitive understanding is to implement a machine learning algorithm from scratch! (2) Knowing how to process/manipulate the data to be ML ready. Now, the skills required for these two steps are admittedly a bit disparate, however, the skills needed to successfully apply ML is inherently interdisciplinary - so I think speaking on these two topics together is rather fitting. We will start with talking a lot about the data and steps we'll take to prepare it, and then we're going to totally switch gears and dive into the math at hand and even get into some code. It may seem like an abrupt switch, so I offer that as a brief disclaimer.\\

So we will start by talking a bit more about what machine learning is.. defining some terms, things like that, then move into the steps in the machine learning process. As mentioned, we can't talk about machine learning without getting our hands dirty with some data, and so we'll walk-through a lot of that. After that, we will shift our focus on the math that fuels machine learning and even code a Supprt Vector Machine from scratch. At the very end, we'll take a look Python's ML tools and apply it to a Grand Rapids Car crash dataset, and that dataset will serve as a case study that will flow through the entire talk. It's going to be a lot of fun.}
...
\end{enumerate}
\end{frame}
\note{What has caused this spike? The math that powers machine learning algorithms has been around for quite a few years... so what's changed? It really boils down to two things.\\
