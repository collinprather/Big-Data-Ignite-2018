...
\end{multicols}
\end{frame}
\note{Before we get going here, I'd like to direct you to colab.research.google.com to get things set up for the live coding demo that will take place in the second half of our time today. Follow the instructions on my github at...}

\section{Machine Learning Overview}
\frame{\titlepage}

\frame{\tableofcontents}
\note{Good morning, and thank you all for making it out this early! These conference days are long, and to be here bright and early on day 3 is no joke, so I'm glad you're here.\\
...
Before we really dive in, I want to offer some motivation for what we'll be talking about today. We're going to take a detailed look at the mathematical foundations that enable machines to learn from data. You may very well ask: Why? What's the point? You see we have modern software, much of it free that acts as like a magic black box: You put your data in one side and you get intelligent predictions out on the other side. This "off-the-shelf" software has made machine learning incredibly more accessible. Which is a great thing.\\

I would assert that having an understanding of the trade-offs and complexities associated with the math that supports machine learning orients you in the best possible position to successfully apply machine learning to solve problems. This indeed is a difficult thing to do.\\
In addition, having an understanding of the inner workings of ML can give you direction to advance to more complex algorithms and other subsets of artificial intelligence. It's a huge asset.\\

Today, we're going to start with a brief overview of what machine learning is, then, we'll take a look at a dataset collected by the GR Police Department, and frame the machine learning problem that we'll tackle at the end. After peaking at that data, we'll totally switch gears and dive into the math at hand, and build our model from scratch. At the very end, we'll circle back around and apply python's machine learning tools to our Grand Rapids Crash dataset and make some interesting predictions. That will be a lot of fun.}
...
\note{All this talk begs the question, what is machine learning? Even according to the experts, the exact definifion of the field of machine learning is a bit fuzzy, but  As early as 1959, Arthur Samuel was famously quoted as defining machine learning as...\\
ML techniqes can be applied to a wide range of problems across many industries. In fact, ML has become ubiquitous in our everyday lives\\
* Siri/ Amazon Alexa\\
...
\begin{frame}{Types of Machine Learning Models}
...
\end{tabular}
\end{frame}
\note{Given its diverse use-cases, not all ML looks alike. There are hundreds of potential models to choose from, and trade-offs associated with each. The good news is, with each type of machine learning problem comes its own little subset of algorithms -- so this narrows down our search considerably. That's what these little bullet points are, different ML algorithms\\
Regression problems are concerned with predicting a continuous variable (a real number). \textbf{examples: predicting housing prices, predicting someone's age}\\
Classification problems on the other hand are concerned with separating or classifying the data, predicting a discrete variable. \textbf{examples: Predicting whether or not a customer will cancel their subscription, predicting if cancer cells are malignant or benign}.\\
Supervised ML problems refer to the case when our training data has both input values (characteristics about the customer) and the associated label (whether the customer canceled their subscription). In unsupervised ML problems, we don't have access to that associated label... all that we have is the data! So we often use unsupervised learning to cluster our data, sometimes even generating those labels.\\
The type of ML problem that we're dealing with has a lot to do with the way that we frame our initial problem. More on that in a moment...}
...
\end{center}
\end{frame}
\note{This may not be a surprise to you, but the world's google search history reflects a steady increase in interest in machine learning and other related terms like Data Science, Big Data, and Artificial Intelligence.\\
...
\note{Now that we have some motivation for what machine learning is and what it can do, let's move into the steps of the machine learning process.}
...
This is a perliminary step of any machine learning project, asking the questions, can this problem be solved with data? Can we learn from historical data to make predictions about new data? These are the questions we will try to answer.}
...
\end{frame}
\note{Step 1! Obtaining the data you'll need looks very different depending on what domain you're working in. In some instances, it can be fairly simple and straightforward, for example, In a business context, most often it will require querying some sort of internal database. Could also be downloading a csv file. In other instances, it may require a bit more creativity\\
...
\textbf{note that this is a supervised classification problem}}
...
%	\end{itemize}
%\end{frame}
%\note{ Now, if you're just emailed a csv file, this step is especially crucial, and it may take you some time to explore the data, get a feeling for what you're dealing with. If you are analyzing data that you work with day in and day out, this "exploration" process may be a bit more implicit.\\
...
%\end{center}
%\end{frame}
%\note{We may also be interested in hit and runs...\\
...
%	\vdots
%\end{matrix}\right]$}
%\only<2-3>{$\hspace{10pt}\Longrightarrow\hspace{10pt}f(hour) = \frac{2\cdot\pi\cdot(\text{hour})}{24}$}
...
%				0.001\\
%				\vdots
%			\end{matrix}\right]}_{cos(f(hour))}$}}
%\end{frame}
%\note{Okay, so we have this new vector, but it doesn't look too helpful at the moment. well, we take that output and map it to two separate vectors, a sin and cos transformed vector. }
%
%
%\begin{frame}{Feature Engineering: \textit{transforming "hour" variable}}
%	\includegraphics[scale=.37]{Figures/hour}
%\end{frame}
%\note{So.. why did we do all of that? Well, when we transfer that all into code, and graph the two vectors on the x-y plane, we see that they make a perfect circle... kind of like a clock..\\
%This is exactly what we want. We've mapped our single hour column to two separate hour colums (think of them like x and y coordinates for each hour). This cyclic representation of time conveys the hour of the car crash in a way that has meaning to the algorithm. This is a really important aspect of feature engineering.}
...
%	
%	\begin{itemize}
%		\item<2-> Numerical variables (Quantitative)
%		\begin{itemize}
...
%	\vspace{5pt}
%	
%		\item<3-> Categorical variables (Qualitative)
%		\begin{itemize}
...
%Categorical variables are qualitative, in our case, we have some binary categorical variables: like whether or not the crash was a hit and run, whether or not a mototcycle was involved. It's either one or the other. Variables can of course have multiple categories.}
...
%\end{frame}
%\note{One critical aspect to take into consideration when choosing a model is called the bias-variance tradeoff. These dartboards create a great analogy for understanding the bias-variance trade-off. I got this analogy from a paper written by Pedro Domingos, I'm not positive that he was the creator, but regardless, I think it's incredibly useful. Bias and Variance have some specific definitions in statistics, but in this context, they are words that we use to describe the behavior of a machine learning model.\\
%For example, If I were to say that our model has high variance, then that means that its predictions would vary significantly if we were to train it on a different sample of our dataset.\\
...
%Generally, bias and variance are inversely related, and controlled by the complexity of our model. If it is a highly complex model, like a neural network, it is likely to be high in variance. If it's a relatively simple model like a linear regression, it's likely to be more biased. Regardless, when applying ML, this is something to be aware of.}



\section{Building a Support Vector Machine from Scratch}

\note{\textbf{Moving to SVMscratch.ipynb}\\
Here's what we're going to do: first we'll be going through a general example of building an svm from scratch on a generated dataset, (which will take the bulk of our time together). then apply some of Python's ml tools to our crash data set to predict whether or not a car crash was caused by a drunk driver!}
...
The first time you run code you may get this pop-up warning you that the notebook wasn't authored by google... that's because it was authored by me, so you can click right through that\\
...
\note{\textbf{How do we find the optimal separating hyperplane?}:\\
...
\note{\textbf{Understanding the Definition of a hyperplane:}\\
...
\textbf{Quick refresher on the dot product:}\\
...
In order to defend against the outlier, we will introduce a regularization term to the end of our hinge loss function, parameterized by the coefficient $\lambda$.}

\note{ \textbf{Regularization term as key to bias-variance tradeoff}\\
The regularization term does more than just protect against outliers though. This provides a great opportunity to take a brief detour through the bias-variance tradeoff:\\
The bias-variance tradeoff is a consideration that permeates all types of ML and is a critical aspect to take into consideration when choosing a model. These dartboards create a great analogy for understanding the bias-variance trade-off. Bias and Variance have some specific definitions in statistics, but in this context, they are words that we use to describe the behavior of a machine learning model.\\
For example, If I were to say that our model has high variance, then that means that its predictions would vary significantly if we were to train it on a different sample of our dataset.\\
If I were to say that our model was highly biased, then that would mean that it's predictions are very consistent, regardless of the sample of data that we trained it on. (that does not necessarily mean accurate, as we see in the bottom left corner.)\\
Generally, bias and variance are inversely related, and controlled by the complexity of our model. If it is a highly complex model, like a neural network, it is likely to be high in variance. If it's a relatively simple model like a linear regression, it's likely to be more biased. Regardless, when applying ML, this is something to be aware of.
The $\lambda$ coefficient acts as a tuning parameter that helps to balance the bias-variance tradeoff. \\
Generally, as the value for lambda gets smaller and smaller (approaches 0), the margin grows wider (to the point of misclassifying data points). As the value for lambda gets larger and larger, the margin will get smaller and smaller (to the point of no misclassified data points.)}
...
\note{\textbf{SGD from Scratch}}


\section{Exploring Scikit-Learn and applying to GR Crash dataset}
\note{\textbf{Applyng Sci-kit learn to predict drunk drivers}}

\note{\textbf{Conclusion}
Now, Admittedly, I found and explored this dataset for the purpose of presenting here, (look up notes from michael to better explain this?)\\
But truly, the challenge in ML is not the math (the machine will do it for you), and often it's not the data itself (as aforementioned, it's becoming increasingly more accessible). The challenge is often building a ML model that is stable, has "business`` value, and is able to be used in production.\\
I thought that maybe I can leave you with this challenge: If you knew, as we've shown today, that given data from the Police's U10 reports, you could accurately predict if causes were caused by drunk drivers, how would that be helpful? What could that be used for? How could we apply it/put it into use?\\
