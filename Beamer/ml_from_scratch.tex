...
And here is a snapshot of data set. Note that for each data point (each row/each dot), we have an $x_1$ value, and $X_2$ value, and a $y$ value (which represents the target variable, which we are trying to predict). It may seem weird to call them $x_1$ and $x_2$, but we'll address why we do that in a little bit.}
...
Now, armed with this ability to calculate how far away each data point, we can quantify how wrong each of our predictions are!\\
